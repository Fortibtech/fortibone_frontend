...
export interface CartItem {
  ...
  variantId: string;
  name: string;
  price: number; // number obligatoire
  quantity: number;
...
interface CartState {
  ...
  // Mettre à jour la quantité (utile pour + / -)
  updateQuantity: (
    productId: string,
    variantId: string,
    quantity: number
  ...
  // Toggle classique : présent → retire, absent → ajoute 1
  toggleItem: (item: Omit<CartItem, "id" | "quantity">) => void;

  // Vérifier si un produit+variante est déjà dans le panier
...
export const useCartStore = create<CartState>()(
  persist(
    ...
    {
      name: "cart-storage",
      storage: createJSONStorage(() => AsyncStorage), // ← LIGNE MAGIQUE
    }
  )
);
