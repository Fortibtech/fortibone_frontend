...
export default function RestaurateurOrderDetailPage({ params }: { params: { id: string } }) {
        ...
        try {
            ...
            if (errorMessage.toLowerCase().includes('solde insuffisant')) {
                if (window.confirm(`${errorMessage}\n\nVoulez-vous recharger votre portefeuille ?`)) {
                    router.push('/dashboard/restaurateur/wallet/deposit');
                }
            }
        } finally {
            setPaying(false);
    ...
    if (loading) {
        return (
            <ProtectedRoute requiredProfileType="PRO">
                <DashboardLayout businessType="RESTAURATEUR" title="Chargement...">
    ...
    if (error || !order) {
        return (
            <ProtectedRoute requiredProfileType="PRO">
                <DashboardLayout businessType="RESTAURATEUR" title="Erreur">
    ...
    return (
        <ProtectedRoute requiredProfileType="PRO">
            <DashboardLayout businessType="RESTAURATEUR" title={`Commande #${order.orderNumber}`}>
                <div className={styles.container}>
                    {/* Header */}
                    <div className={styles.header}>
                        <button onClick={() => router.push('/dashboard/restaurateur/achats/orders')} className={styles.backButton}>
                            ←
                        </button>
