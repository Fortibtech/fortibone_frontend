...
const CreateBusinessFournisseur: React.FC = () => {
  ...
  // On force le type FOURNISSEUR + valeurs par défaut
  const business = {
    ...
    sectorId: businessData.sectorId || "", // ← Nouveau champ
  ...
  // États pour les secteurs dynamiques
  ...
  useEffect(() => {
    loadCurrencies();
    loadSectors(); // ← Chargement des secteurs
  }, []);
  ...
  const loadSectors = async () => {
    try {
      ...
      const data = await getAllSectores("FOURNISSEUR");
      setSectors(data);
  ...
  // Filtre des secteurs selon la recherche
  useEffect(() => {
    if (!searchQuery.trim()) {
  ...
  // Gestion du texte des zones de livraison
  useEffect(() => {
    if (
  ...
  const validateAndSave = async () => {
    ...
    try {
      const cleanPayload: any = {
        ...
        sectorId: business.sectorId, // ← Envoi de l'ID
      ...
      try {
        ...
        console.warn("Impossible de sélectionner le commerce", err);
      }
      ...
      Alert.alert(
        ...
        [
          {
            text: "Continuer",
            onPress: () => router.replace("/(professionnel)"),
          },
        ],
        { cancelable: false }
      );
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      >
        ...
        {/* MODAL SECTEUR D’ACTIVITÉ (dynamique) */}
