import { BusinessesService } from "@/api/services/businessesService"; // ✅ adapte si besoin
...
export default function OrdersScreen() {
  ...
  const renderItem = ({ item }: { item: Business }) => (
    <TouchableOpacity
      style={styles.card}
      onPress={async () => {
        await BusinessesService.selectBusiness(item); // ✅ garde la sélection
        router.push(`/(orders)/details/${item.id}`); // ✅ redirige vers la liste des commandes
      }}
    >
      <Ionicons
    ...
    <View style={{ flex: 1 }}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerDescription}>
          Consultez et gérez les commandes de chaque entreprise.
        </Text>
      </View>
